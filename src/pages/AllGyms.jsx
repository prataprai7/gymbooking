...
const RadioButton=({label, selected=false, onChange=()=>{ }})=>{
    return(
        <label className="flex gap-3 items-center cursor-pointer mt-2 text-sm">
            <input type="radio" name='shortOption' checked={selected} onChange={()=>onChange(
            label)}/>
            <span className='font-light select-none' >{label}</span>
        </label >
    )
    ...
    return(
        ...
        xl:px-32' >
            ...
            <div className='flex flex-col items-start text-left'>
                ...
                max-w-174'>Take advantage of our limited-time offers and special packages 
            ...
            {roomsDummyData.map((room)=>(
                ...
                border-b border-gray-300 last:pb-30 last:border-0'>
                    <img onClick={()=>{navigate('/rooms/${room._id}'); scrollTo(0,0)}}
                     src={room.images[0]} alt="gyms-img" title='View Gym Details' 
                    className='max-h-65 md:w-1/2 rounded-xl shadow-lg object-cover cursor-pointer'/>
                    <div className='md:w-1/2 flex flex-xpl gap-2'>
                        <p className='text-grey-500'>{room.hotel.city}</p>
                        <p onClick={()=>{navigate('/rooms/${room._id}'); scrollTo(0,0)}}
                         className='text-grey-800 text-3xl font-playfair cursor-pointer'>{room.hotel.name}</p>
